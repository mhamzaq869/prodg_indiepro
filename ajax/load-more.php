...
elseif (!is_numeric($_POST['last_id']) || $_POST['last_id'] < 0) {
   ...
   //              }
		...
		$db->where('privacy', 0);
		$videos = $db->where('time', time() - 172800, '>')->where('id', $id, '>')->where('is_movie',0)->orderBy('views', 'DESC')->get(T_VIDEOS, 40);
		if (!empty($videos)) {
		    $len = count($videos);
		    foreach ($videos as $key => $video) {
		        $video = PT_GetVideoByID($video, 0, 0, 0);
		...
		$videos = $db->where('user_id', $user_id)->where('id', $id, '<')->where('is_movie',0)->orderBy('id', 'DESC')->get(T_VIDEOS, 40);
		if (!empty($videos)) {
		    $len = count($videos);
		    foreach ($videos as $key => $video) {
		        $video = PT_GetVideoByID($video, 0, 0, 0);
		...
		if (!empty($videos)) {
		    ...
		    foreach ($videos as $key => $video) {
		        ...
		        if ($_POST['paid_sort'] == 'rented_videos' || $_POST['paid_sort'] == 'rented_movies') {
		            $file = 'rent';
		            $rent_video_time = date('d-F-Y',$video->rent_video_time + (60*60*24*2));
		            $rent_video_time_start = date('d-F-Y',$video->rent_video_time);
		        }
