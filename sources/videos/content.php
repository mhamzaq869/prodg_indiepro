...
if ($page == 'trending') {
    ...
    // pagination system 
    $videos = $db->where('privacy', 0)->where('user_id',$pt->blocked_array , 'NOT IN')->where('time', time() - 172800, '>')->where('is_movie',0)->where('live_time',0)->where('approved',1)->orderBy('views', 'DESC')->objectbuilder()->paginate(T_VIDEOS, $pt->page_number);
    ...
    // pagination system 
    $videos = $db->where('privacy', 0)->where('user_id',$pt->blocked_array , 'NOT IN')->where('is_movie',0)->where('live_time',0)->where('approved',1)->orderBy('id', 'DESC')->objectbuilder()->paginate(T_VIDEOS, $pt->page_number);
    $pt->total_pages = $db->totalPages;
    // pagination system 
} 
else if ($page == 'live') {
    ...
    // pagination system 
    $videos = $db->where('privacy', 0)->where('user_id',$pt->blocked_array , 'NOT IN')->where('is_movie',0)->where('live_time',0,'>')->where('approved',1)->orderBy('live_time', 'DESC')->objectbuilder()->paginate(T_VIDEOS, $pt->page_number);
    $pt->total_pages = $db->totalPages;
    // pagination system 
} 
    ...
    // pagination system 
    $videos = $db->where('privacy', 0)->where('user_id',$pt->blocked_array , 'NOT IN')->where('is_stock',1)->where('approved',1)->orderBy('live_time', 'DESC')->objectbuilder()->paginate(T_VIDEOS, $pt->page_number);
    $pt->total_pages = $db->totalPages;
    // pagination system 
} 
    ...
    if ($pt->cat_type == 'all_time') {
        // pagination system 
        $videos = $db->where('privacy', 0)->where('user_id',$pt->blocked_array , 'NOT IN')->where('is_movie',0)->where('live_time',0)->where('approved',1)->orderBy('views', 'DESC')->objectbuilder()->paginate(T_VIDEOS, $pt->page_number);
        $pt->total_pages = $db->totalPages;
        
        // pagination system 
    }
    else{
        $videos = $db->rawQuery('SELECT video_id, COUNT(*) AS count FROM '.T_VIEWS.' v WHERE `time` >= '.$start.' AND `time` <= '.$end.'  AND (SELECT id FROM '.T_VIDEOS.' WHERE id = v.video_id AND is_movie = 0 AND live_time = 0 AND privacy = 0 AND user_id NOT IN ('.implode(",", $pt->blocked_array).') AND approved = 1) = video_id GROUP BY video_id ORDER BY count DESC LIMIT '.$page_num.','.$limit);

        $rows = $db->rawQuery('SELECT COUNT(*) FROM '.T_VIEWS.' v WHERE `time` >= '.$start.' AND `time` <= '.$end.'  AND (SELECT id FROM '.T_VIDEOS.' WHERE id = v.video_id AND is_movie = 0 AND privacy = 0 AND live_time = 0 AND user_id NOT IN ('.implode(",", $pt->blocked_array).') AND approved = 1) = video_id GROUP BY video_id');
    ...
    if (!empty($_GET['id'])) {
        if (in_array($_GET['id'], array_keys($categories))) {
            ...
            // pagination system 
            $videos = $db->where('privacy', 0)->where('user_id',$pt->blocked_array , 'NOT IN')->where('category_id = "' . $cateogry.'"')->where('is_movie',0)->where('live_time',0)->where('approved',1)->orderBy('id', 'DESC')->objectbuilder()->paginate(T_VIDEOS, $pt->page_number);
            $pt->total_pages = $db->totalPages;
            // pagination system 
