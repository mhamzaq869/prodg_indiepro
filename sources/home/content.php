...
if ($pt->theme_using == 'default') {
    ...
    $video_obj = $db->where('featured', '1')->where('user_id',$pt->blocked_array , 'NOT IN')->where('is_movie', 0)->where('privacy', 0)->where('live_time', 0)->where('approved',1)->orderBy('RAND()')->getOne(T_VIDEOS);
...
if (empty($get_video)) {
    ...
    $db->where('converted', '2','<>');
    $get_video = PT_GetVideoByID($db->where('privacy', 0)->where('user_id',$pt->blocked_array , 'NOT IN')->where('is_movie', 0)->where('live_time', 0)->where('approved',1)->orderBy('id', 'DESC')->getOne(T_VIDEOS), 0, 1, 0);
}
...
if (!empty($pro_users)) {
    ...
    $db->where('privacy', 0);
    $trending_data = $db->where('is_movie', 0)->where('user_id',$pt->blocked_array , 'NOT IN')->where('approved',1)->where('live_time',0)->orderBy('views', 'DESC')->get(T_VIDEOS, $limit);
}
...
if (!empty($pro_users)){
    ...
    $top_data = $db->where('is_movie', 0)->where('user_id',$pt->blocked_array , 'NOT IN')->where('approved',1)->where('live_time',0)->get(T_VIDEOS, 4);
}

if (empty($top_data)) {
    $db->where('privacy', 0);
    $top_data = $db->where('is_movie', 0)->where('user_id',$pt->blocked_array , 'NOT IN')->where('approved',1)->where('live_time',0)->orderBy('views', 'DESC')->get(T_VIDEOS, $limit);
}
...
if ($pt->config->live_video == 1) {
    if (!empty($pro_users)){
        ...
        $live_data = $db->where('is_movie', 0)->where('user_id',$pt->blocked_array , 'NOT IN')->where('approved',1)->where('live_time',0,'>')->get(T_VIDEOS, 4);
    }

    if (empty($live_data)) {
        $db->where('privacy', 0);
        $live_data = $db->where('is_movie', 0)->where('user_id',$pt->blocked_array , 'NOT IN')->where('approved',1)->where('live_time',0,'>')->orderBy('live_time', 'DESC')->get(T_VIDEOS, $limit);
    }
...
if (!empty($pro_users)) {
    ...
    $db->where('privacy', 0);
    $db->orderBy('id', 'DESC');
    $latest_data = $db->where('is_movie', 0)->where('user_id',$pt->blocked_array , 'NOT IN')->where('approved',1)->where('live_time',0)->get(T_VIDEOS, $limit);
}

if (empty($latest_data)) {
    $db->where('privacy', 0);
    $latest_data = $db->where('is_movie', 0)->where('user_id',$pt->blocked_array , 'NOT IN')->where('approved',1)->where('live_time',0)->orderBy('id', 'DESC')->get(T_VIDEOS, $limit);
}
...
foreach ($categories as $cat_key => $cat_name) {
    if (IS_LOGGED == true && !empty($pt->user->fav_category)) {
        if (in_array($cat_key, $pt->user->fav_category)) {
            ...
            $db->where('privacy', 0);
            $db->orderBy('id', 'DESC');
            $pt->cat_videos = $db->where('is_movie', 0)->where('user_id',$pt->blocked_array , 'NOT IN')->where('approved',1)->where('live_time',0)->get(T_VIDEOS, 10);
